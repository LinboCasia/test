...
void printUsage() {
  std::cout
    ...
    << "  get-dict            get all word->vectors file\n"
    << std::endl;
}
...
void printGetDictUsage() {
  std::cout
    << "usage: get-dict  <model> <outpath> \n\n"
    << "  <model>      model filename\n"
    << "  <outpath>  output data filename \n"
    << std::endl;
}
...
void test(int argc, char** argv) {
  int32_t k;
  std::string predOutputPath;
  if (argc == 4) {
    k = 1;
    predOutputPath = "../data/result/fasttext_label.txt";
  } else if (argc == 5) {
    k = atoi(argv[4]);
    predOutputPath = "../data/result/fasttext_label.txt";
  } else if (argc == 6) {
    k = atoi(argv[4]);
    predOutputPath = std::string(argv[5]);
  } else {
    printTestUsage();
    exit(EXIT_FAILURE);
  }
  ...
  if (infile == "-") {
    fasttext.test(std::cin, k, predOutputPath);
  } else {
    ...
    fasttext.test(ifs, k, predOutputPath);
    ifs.close();
  }
  exit(0);
}
...
void getDict(int argc, char** argv){
