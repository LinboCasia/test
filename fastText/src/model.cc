...
void Model::computeOutputSoftmax(Vector& hidden, Vector& output,double& z) const {
...
void Model::computeOutputSoftmax() {
  double z = 0.0; 
  computeOutputSoftmax(hidden_, output_,z);
}
...
void Model::predict(const std::vector<int32_t>& input, int32_t k,
                    ...
                    Vector& hidden, Vector& output, double& z) const {
  assert(k > 0);
  heap.reserve(k + 1);
  ...
  if (args_->loss == loss_name::hs) {
    ...
    findKBest(k, heap, hidden, output, z);
  }
...
void Model::predict(const std::vector<int32_t>& input, int32_t k,
                    std::vector<std::pair<real, int32_t>>& heap) {
  double z=0.0;
  predict(input, k, heap, hidden_, output_,z);
  //std::cout << "llll " << output_[0]<<"--" <<output_[1] << std::endl;
}


void Model::predict(const std::vector<int32_t>& input, int32_t k,
                    std::vector<std::pair<real, int32_t>>& heap,double& z) {
  predict(input, k, heap, hidden_, output_,z);
  //std::cout << "llll " << output_[0]<<"--" <<output_[1] << std::endl;
}
